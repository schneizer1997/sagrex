...
global $dbcheque;
function insertcheck($checknos,$payto,$amount,$inwords,$tdate){
	//error_reporting(E_ALL);
	global $dbcheque;
	$objResponse= new xajaxResponse();
	$sqlselect = "SELECT *FROM cheq_info where cheq_no = '".$checknos."' ";
	$result = $dbcheque->query($sqlselect);
	//die($sqlselect);
	if($result->num_rows > 0){
		$objResponse->addAlert('Check No. is already exist.');
	}
	else {
		$sqlcheckinsert = "INSERT INTO cheq_info(cheq_no,pay_to,amount,amount_words,trans_date,date_modified,isdelete)VALUES('".addslashes($checknos)."','".addslashes($payto)."','".addslashes($amount)."','".($inwords)."','".addslashes($tdate)."','".date("Y/m/d")."','1')";
	//die($sqlcheckinsert);
			if ($dbcheque->query($sqlcheckinsert) == TRUE){
	...
	function insertpayto($payto_name){
	//error_reporting(E_ALL);
	global $dbcheque;
	//$response = false;
	$objResponse= new xajaxResponse();
	$sqlselect = "SELECT *FROM payto_tbl where payto_name = '***".$payto_name."***' ";
	$result = $dbcheque->query($sqlselect);
	//die($sqlselect);
	//echo "var s = '';";
	if($result->num_rows > 0){
		//echo "<script> respo(); </script>";
		$objResponse->addAlert('Name is already exist.');
		/*$response = false;*/
	}
	else {
		$sqlcheckinsert = "INSERT INTO payto_tbl(payto_name,date_modified,status)VALUES('***".addslashes($payto_name)."***','".date("Y/m/d")."',1)";
	//die($sqlcheckinsert);
			if ($dbcheque->query($sqlcheckinsert) == TRUE){
				/*$response = true;
				return $response;*/
	...
	function deletepayto($id){
		
		global $dbcheque;
		$objResponse= new xajaxResponse();
		//var_dump($vehinfo_id);
		$sqlupdatepay = "UPDATE payto_tbl SET status = 0 WHERE payto_id = '".$id."'";
					//die($sqlmaterial);
					//var_dump($sqlupdateitem);
		if ($dbcheque->query($sqlupdatepay) == TRUE){
	...
	function updatecheckinfo($checkid,$checknos,$payto,$amount,$inwords,$tdate){
		
		global $dbcheque;
		$objResponse= new xajaxResponse();
		//var_dump($vehinfo_id);
		$sqlupdatecheck = "UPDATE cheq_info SET cheq_no = '".addslashes($checknos)."',pay_to = '".addslashes($payto)."',amount = '".addslashes($amount)."',amount_words = '".($inwords)."',trans_date = '".addslashes($tdate)."',date_modified = '".date("Y/m/d")."' WHERE cheq_id = '".$checkid."'";
					//die($sqlmaterial);
					//var_dump($sqlupdateitem);
		if ($dbcheque->query($sqlupdatecheck) == TRUE){
				//$objResponse->addAlert('Item has been updated.');
		}
		else{
			$objResponse->addAlert('Error');
