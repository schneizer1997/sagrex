...
echo '</table>';
echo '</form>';*/
$sql = "SELECT *FROM prod_rms_info where status = 1";
...
if ($result->num_rows > 0){
	while ($row=$result->fetch_assoc()){
		...
		$prod_info_id= preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', $row['prod_rms_info_id']);
		$designation= preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', $row['designation']);
		$repairtype=preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', $row['repair_type']);
		$desc=$row['descr'];
		...
		echo '<td  style = "width:90px;">'.$repairtype.'</td>';
