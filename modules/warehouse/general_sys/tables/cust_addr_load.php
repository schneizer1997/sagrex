...
echo '<form action = "rrsis_items.php" method = "GET">';
echo '<table class="table table-striped header-fixed table-bordered table-hover table-info " id ="tblcust_addr" cellspacing="0" style= "width:100%;table-layout: fixed;margin-bottom: -15px;overflow-y: auto; height: 100px;">';
echo '<div>';
...
echo '<th>ID</th><th>Name</th><th>Address</th><th>Action</th>';
echo '</tr>';
echo '</div>';
echo '</thead>';
echo '</div>';
...
if ($result->num_rows > 0){
	while ($row=$result->fetch_assoc()){
		...
		echo '<tr>';
		echo '<td id = "txtmyid'.$c.'" style= "">'.$cust_id.'</td>';
		echo '<td id = "txtdescription'.$c.'" style= "">'.$cust_name.'</td>';
		echo '<td id = "txtunit'.$c.'" style= "">'.$cust_addr.'</td>';
		/*echo '<td><button type = "button" id  = "btnselectitem'.$c.'" name = "btnselectitem" class = "btnselectitem btn btn-success" onclick=\'parent.selectitems("'.$myid.'","'.$description.'", "'.$unit.'")\'><i class="fa fa-plus" style="margin-right: 5px;"></i>ADD</button>
		</td>';*/
		echo '</tr>';
		$c++;
	}
