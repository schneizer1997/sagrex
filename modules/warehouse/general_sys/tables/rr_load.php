...
$sql = "SELECT rrno, cust_name,datecreated,addr_name FROM rrinfo LEFT JOIN tblcustomer ON tblcustomer.cust_id = rrinfo.vendor_id LEFT JOIN tbladdress ON tbladdress.addr_id = rrinfo.addr_id";
...
if ($result->num_rows > 0){
	...
	while ($row=$result->fetch_assoc()){
		...
		echo "<tr>";
		echo '<td>'.$row['rrno'].'</td>';
		echo '<td style= "width:200px;">'.$row['cust_name'].'</td>';
